        ...
        json={
            ...
            "messages": [{
                ...
                "content": (
                    ...
                    f"(보도자료, 공시, 뉴스, 사업 공고 등)가 있는지 확인해서 정부 보도자료 스타일 "
                    f"개조식으로 정리해줘. 형식은 정확히 이렇게: 첫 줄 '□ [핵심 요지, 명사형 종결]', "
                    f"그 아래 1~2줄 '- [부연 사실, 명사형 종결]'(동그라미나 다른 기호 말고 반드시 "
                    f"대시(-) 사용). 완결형 문장(~습니다/됩니다/입니다)이나 화살표(→)·긴 대시(—) "
                    f"연결은 쓰지 말 것. 반드시 검색 결과에 실제로 있는 내용만 근거로 삼고, 매입 "
                    f"의사를 밝힌 근거가 검색 결과에 없으면 지어내지 말고 '- 공개 자료에서 매입 "
                    f"의사 직접 확인 못함'이라고 솔직히 적어줘.\n\n{context}"
                ),
            }],
            "max_tokens": 300,
            "temperature": 0.2,
    ...
    try:
            ...
            json={
                ...
                "messages": [{
                    ...
                    "content": (
                        ...
                        '형식: {"companies": [{"name": "회사명", '
                        '"role": "사업 내용 한 줄(명사형 종결, 예: \\"폐배터리 수거·재활용\\")", '
                        '"loc": "지역 (모르면 \\"—\\")", "band": "reuse|ess|material|collect 중 하나 '
                        "(reuse=EV 재제조/재사용, ess=2차사용·ESS, material=소재회수·재활용, "
                        'collect=단순 수거·매입 중개)", '
                        '"fact": "검색 결과에 실제로 있는 근거 한 줄(명사형 종결, 대시/완결형 문장 금지)", '
                        '"link": "이 회사 설명이 나온 검색 결과의 괄호 안 URL을 그대로 복사(지어내지 '
                        '말고, 모르면 빈 문자열)"}]}. role/fact는 반드시 정부 보도자료 개조식(예: '
                        "\"~수거\", \"~보유\", \"~계약 체결\")으로 쓰고 '~습니다/됩니다/입니다' 같은 "
                        "완결형 문장으로 쓰지 마. 검색 결과에서 확인되지 않는 내용은 절대 지어내지 "
                        f"말고, 근거가 빈약한 회사는 목록에서 빼줘. 최대 8곳까지만.\n\n{context}"
                    ),
                }],
                "max_tokens": 1500,
                "temperature": 0.1,
        ...
        for c in companies:
            ...
            buyers.append({
                ...
                # "□" 접두어를 붙여 pdf_report._render_freeform_gov가 개조식 구조로
                # 인식하게 한다(그냥 문장이면 자유 텍스트로 보고 완결형 종결만 다듬음).
                "why": f"□ {c.get('role') or '실시간 검색으로 확인된 매입처'}",
                "fact": c.get("fact") or "실시간 검색 결과 기반, 상세 근거 별도 확인 필요",
                "source_url": link,
            })
