...
BUYERS = [
    # ---------------- 재사용(EV 재제조)급 ----------------
    {
        "name": "현대글로비스", "emoji": "🚛", "loc": "전국 (수거·회수 네트워크 전국 운영)",
        "role": "폐배터리 회수·유통 (현대차그룹)",
        "band": "reuse", "accepts": ["1등급"],
        "why": "□ 현대차그룹 순환경제 체계 적합\n"
               "-　현대글로비스 수거, 현대모비스 재제조 체계 활용\n"
               "-　재제조 가능한 고SOH 팩일수록 높은 가치 인정",
    ...
    },
    {
        ...
        "why": "□ 공공 경쟁입찰 방식 적용\n"
               "-　시황 호조 시 예정가 3~4배까지 낙찰가 형성\n"
               "-　낙찰가 변동폭 크고 입찰 일정 제한적",
    ...
    },
    {
        ...
        "role": "재사용 + 재활용 통합",
        "band": "reuse", "accepts": ["1등급", "2등급"],
        "why": "□ 재사용·재활용 통합 처리 가능\n"
               "-　수거·재사용과 블랙파우더 추출(재활용) 동시 수행\n"
               "-　등급 혼재·혼합 물량도 단일 창구 처리 가능",
    ...
    # ---------------- 2차사용(ESS)급 ----------------
    {
        ...
        "why": "□ 진단·등급·재사용처 흐름과 가장 유사한 실사례\n"
               "-　국내 최초 재사용 전담 공장 운영\n"
               "-　SOH 기반 등급 분류 후 ESS·UPS 재제조",
    ...
    # ---------------- 재활용(소재회수)급 ----------------
    {
        ...
        "why": "□ 국내 최대 규모 재활용 처리 체계\n"
               "-　국내 유일 전·후처리 통합 설비 운영\n"
               "-　재사용 곤란 팩의 확실한 최종 처분처",
    ...
    },
    {
        ...
        "band": "material", "accepts": ["1등급", "2등급", "3등급"],
        "why": "□ 대기업 계열 대량·장기 계약 적합\n"
               "-　니켈·코발트·리튬 자체 개발 기술로 회수\n"
               "-　대량·장기 계약 물량 처리에 유리",
    ...
    },
    {
        ...
        "role": "재활용 (블랙파우더 처리)",
        "band": "material", "accepts": ["1등급", "2등급", "3등급"],
        "why": "□ 대형 합작사 기반 처리 규모, 남부권 물류 유리\n"
               "-　포스코·화유코발트·GS 합작사, 블랙파우더 처리 규모 대형\n"
               "-　남부권(전남 여수) 소재로 물류 효율적",
    ...
    },
    {
        ...
        "band": "material", "accepts": ["1등급", "2등급", "3등급"],
        "why": "□ 중규모 물량 처리 적합\n"
               "-　납축전지 리사이클 노하우 리튬으로 확장\n"
               "-　중규모 물량 처리에 유리한 설비 규모",
    ...
    },
    {
        ...
        "role": "재활용",
        "band": "material", "accepts": ["1등급", "2등급", "3등급"],
        "why": "□ 중견 처리사, 중부권 물류 유리\n"
               "-　50년 폐기물 재활용 노하우 보유\n"
               "-　자체 R&D센터 운영, 중부권(충북 충주) 소재",
    ...
    },
    {
        ...
        "band": "material", "accepts": ["1등급", "2등급", "3등급"],
        "why": "□ 대형 물량 소화력, 등급 무관 매입\n"
               "-　LG에너지솔루션과 장기 공급계약 체결\n"
               "-　등급과 무관하게 전량 매입 가능",
    ...
    # ---------------- 수거·매입(중개)급 ----------------
    {
        ...
        "why": "□ 소량·개별 처분에 간편\n"
               "-　정식 허가 수거업체, 전국 출장수거 가능\n"
               "-　중간 유통이라 단가는 낮으나 즉시 처리 용이",
...
# ⚠️ "『사용후배터리 산업 육성법』(2025.10 시행)"은 사실과 달랐다(웹서치로 재검증) - 이 법
# (사용후 배터리의 관리 및 산업육성에 관한 법률)은 2026.5.26 공포, 2027.5.27 시행 "예정"이라
# 아직 시행 전이다. 지금 실제로 적용되는 근거는 자원순환기본법상 순환자원 지정 고시다.
LEGAL_NOTE = ("전기차 사용후 배터리는 「자원순환기본법」에 따른 순환자원 지정 고시 대상으로 "
              "폐기물관리법 규제가 면제되나, 단순 수리·수선·건조·세척을 통한 재사용 등 일반적· "
              "품목별 준수사항을 충족해야 합니다. 「사용후 배터리의 관리 및 산업육성에 관한 "
              "법률」은 2026.5.26 공포, 2027.5.27 시행 예정이니 사전 대비가 필요합니다.")
